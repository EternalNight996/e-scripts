@echo off
@REM author： Eternal
@REM date: 2022/4/30 11:50
@REM describe: WINDOWS与OFFICE各版本激活
@REM 支持winxp win7 win8 win10 win_server office
@REM 备用kms server: kms.cangshui.net kms.moeclub.org
@REM QQ: 2472674814
@REM QQ群：664741025 欢迎讨论

mode con cols=50 lines=25
title Eternal KMS脚本-V1.0
setlocal EnableDelayedExpansion &color 47 & cd /d "%~dp0"
%1 %2
...
:start
echo 本脚本需要右键管理员运行
set /p selectSome=【1】KMS激活Windows 【2】KMS激活Office 【3】清除Windows KMS 【4】清除Office KMS  请输入你的选择:
if /i "%selectSome%"=="1" cls&goto start1
if /i "%selectSome%"=="2" cls&goto start2
if /i "%selectSome%"=="3" cls&goto start3
if /i "%selectSome%"=="4" cls&goto start4
...
set /p selectSome=【A】激活为Office2019版本(仅2019、2021与365版本可选)   【B】激活为Office2016版本（通用）
if /i "%selectSome%"=="a" cls&goto installOffice19
if /i "%selectSome%"=="b" cls&goto installOffice16
...
echo 未找到合适的NT6系统, 或Win2003。
goto office

:winxp
echo 当前Windows xp
set Professional=HTXH6-2JJC4-CDB6C-X38B4-C3GF3
goto windowsstart
...
:start4
set /p selectSome=是否真的要清除Office的KMS激活？【Y】继续   【N】关闭

if /i "%selectSome%"=="y" goto nextun
if /i "%selectSome%"=="n" exit
:nextun
...
:start3
set /p selectSome=是否真的要清除Windows的KMS？【Y】继续   【N】关闭
if /i "%selectSome%"=="y" goto nextunw
if /i "%selectSome%"=="n" exit
:nextunw
slmgr /upk
slmgr /ckms
